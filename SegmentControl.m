...
@property (strong, nonatomic) UIView * bottomView02;


/**
 *  当前选中索引
 */
@property (assign, nonatomic) Segment * currentItem;

/**
 *  将要选中的Item
 */
@property (assign, nonatomic) Segment * nextItem;
...
- (void)layoutSubviews {
    ...
    if (self.titles.count != 0) {
        for (UIView * subview in self.segementScrollView.subviews) {
            if ([subview isKindOfClass:[Segment class]]) {
                Segment * segment = (Segment *)subview;
...
//    self.bottomView02.width = self.currentItem.width;
    // 设置底部条x值
...
- (void)setTitles:(NSArray *)titles {
    ...
    for (int i = 0; i < titles.count; i ++) {
        ...
        Segment * segment = [[Segment alloc] initWithFrame:CGRectMake(itemX, 0, itemW, self.height)];
 ...
 *  标题项点击事件
 */
- (void)itemAction:(Segment *)segment {
    ...
    //点击事件--计算索引
//    NSLog(@"点击 == %ld",(long)self.currentItem.tag);
...
//    self.bottomView02 = [[UIView alloc] initWithFrame:CGRectMake(0, self.height - SegementViewBottomViewHeight, 0, 30)];

    self.bottomView.backgroundColor = color;
//    self.bottomView02.backgroundColor = [UIColor redColor];
//    [self.segementScrollView addSubview:_bottomView02];
...
- (void)setSegementAnimate {
    // 当前选中的Item
    Segment * currentItem = self.currentItem;
    // 标题栏动画
    [UIView animateWithDuration:0.3 animations:^{
        // 判断能否设置居中（除开距离不够的）
...
-(void)scrollViewEndDragging{
    ...
    if (self.scrollCtrlViewScrollStatus == scrollCtrlViewScrollStatusToRight) {
//        NSLog(@"右滑动");
        if (self.Item > 0) {
            self.Item--;
        }
//        NSLog(@"%ld", (long)self.Item);
    }
    else if(self.scrollCtrlViewScrollStatus == scrollCtrlViewScrollStatusToLeft){
//        NSLog(@"左拖动");
        if (self.Item < 4) {
            self.Item++;
        }
...
-(void)segmentCotrolClick{
    _Item = self.currentItem.tag - 1995;
//    NSLog(@"%ld", (long)self.Item);
