...
@interface CompletedViewController ()<DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>{
...
-(void)getCompletedOrder{
    ...
    NSString *url = [API stringByAppendingString:@"Personal/completed"];
    NSDictionary *dic = @{@"shopId":[AppDelegate APP].user.shopId};
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    
    [manager POST:url parameters:dic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"获取已经完成订单 %@", responseObject);
        self.loading = NO;
...
//            [Util toastWithView:self.navigationController.view AndText:@"获取已完成订单失败"];
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSLog(@"获取已经完成订单失败 %@", error);
        [self.tableView.header endRefreshing];
        self.loading = NO;
